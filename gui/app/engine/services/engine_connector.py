...
class EngineConnector(QObject):
    ...
    def stop(self) -> None:
        ...
        if self.process.state() == QProcess.ProcessState.Running:
            ...
            if not self.process.waitForFinished(1000):
                logger.warning("Engine did not exit within 1s; terminating")
    ...
    def send(self, command : str):
        ...
        logger.debug("Sent engine command: %s", command.strip())
    ...
    def _handle_ready_read_stdout(self) -> None:
        ...
        while self.process.canReadLine():
            ...
            if raw_line:
                logger.debug("Engine stdout: %s", raw_line)
    ...
    def _handle_process_finished(
        ...
        logger.info(
            "Engine process finished with exit code %s and status %s",
            exit_code,
            exit_status,
        )
