...
class UCIParser:
    ...
    def parse_line(
        ...
        if not line:
            logger.debug("Skipping empty UCI line")
            return None

        logger.debug("Parsing UCI line: %s", line)
        tokens = line.split()
        cmd = tokens[0]
        ...
        logger.debug("Ignoring unrecognized UCI command: %s", cmd)
        return None
    ...
    def _parse_score(
        ...
        try:
            ...
            logger.warning("Invalid UCI score value: %s", score_str)
